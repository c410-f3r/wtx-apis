...
impl<A> PackAccounts<A> for CloseLookupTableAccounts<A>
where
  A: AccountAddress,
{
  fn len(&self) -> usize {
  ...
  where
    A: AccountAddress,
  {
    let Self { lookup_table_address, authority_address, recipient_address } = self;
    ...
    vec.push(InstructionAccountInput::writable(recipient_address.to_account_address()?))?;
    Ok(())
  }
  ...
  fn unpack_accounts(iter: &mut impl Iterator<Item = A>) -> crate::Result<Self> {
    Ok(Self {
      ...
      recipient_address: next_iter_elem(iter)?,
    })
  }
}

impl TestingInstances for CloseLookupTableAccounts<[u8; 32]> {
  fn min_instance() -> crate::Result<Self> {
    Ok(Self {
      lookup_table_address: [0; 32],
      authority_address: [1; 32],
      recipient_address: [2; 32],
    })
  }
}
