use crate::{
  solana::{misc::next_iter_elem, PackAccounts, TestingInstances},
  AccountAddress,
};
use wtx::collection::Vector;
use crate::blockchain::solana::InstructionAccountInput;

create_account_struct! {
  #[derive(Debug, PartialEq)]
  pub struct SwapParamsAccounts<A> {
    pub token_program: A,
...
impl<A> PackAccounts<A> for SwapParamsAccounts<A>
where
  A: AccountAddress,
{
    fn len(&self) -> usize {
    ...
    A: AccountAddress,
  {
    ...
    vec.push(InstructionAccountInput::none(oracle.to_account_address()?))?;
    Ok(())
  }
...
impl TestingInstances for SwapParamsAccounts<[u8; 32]> {
    fn min_instance() -> crate::Result<Self> {
    Ok(Self {
      token_program: [1; 32],
      token_authority: [2; 32],
      whirlpool: [3; 32],
      token_owner_account_a: [4; 32],
      token_vault_a: [5; 32],
      token_owner_account_b: [6; 32],
      token_vault_b: [7; 32],
      tick_array_0: [8; 32],
      tick_array_1: [9; 32],
      tick_array_2: [10; 32],
