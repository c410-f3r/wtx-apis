...
impl<A> PackAccounts<A> for OpenPositionAccounts<A>
where
  A: AccountAddress,
{
  fn len(&self) -> usize {
    ...
    A: AccountAddress,
  {
    ...
    vec.push(InstructionAccountInput::none(rent.to_account_address()?))?;
  ...
  fn unpack_accounts(iter: &mut impl Iterator<Item = A>) -> crate::Result<Self> {
    Ok(Self {
      ...
      rent: next_iter_elem(iter)?,
      associated_token_program: next_iter_elem(iter)?,
    })
  }
}

impl TestingInstances for OpenPositionAccounts<[u8; 32]> {
  fn min_instance() -> crate::Result<Self> {
    Ok(Self {
      ...
      system_program: [8; 32],
      rent: [9; 32],
      associated_token_program: [10; 32],
    })
  }
}
