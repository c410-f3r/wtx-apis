...
impl<A> PackAccounts<A> for TransferCheckedAccounts<A>
where
  A: AccountAddress,
{
  fn len(&self) -> usize {
  ...
  where
    A: AccountAddress,
  {
    let Self { source, mint, destination, authority, signers } = self;
    vec.extend_from_copyable_slice(&[
      InstructionAccountInput::writable(source.to_account_address()?),
      InstructionAccountInput::writable(mint.to_account_address()?),
...
impl TestingInstances for TransferCheckedAccounts<[u8; 32]> {
